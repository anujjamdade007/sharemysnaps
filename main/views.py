...
def dashboard(request):
    ...
    if response.status_code == 200:
        ...
        messages.error(
            request,
            "Failed to retrieve Google Drive storage information. Consider review the permisions",
        )
...
def create_folder(request):
    ...
    if not access_token:
        messages.error(
            request, "No access token found. Please re-authenticate with Google."
        )
        return redirect("login")
    ...
    if response.status_code == 200:
        ...
        permission_data = {"role": "reader", "type": "anyone", "withLink": True}

        permission_response = requests.post(
            permission_url, headers=headers, json=permission_data
        )
        ...
        if permission_response.status_code == 200:
            ...
            messages.success(
                request,
                f"Folder '{folder_name}' created successfully and is now publicly viewable.",
            )
            return redirect("dashboard")
        else:
            messages.error(
                request,
                f"Failed to set public access for the folder. Error: {permission_response.text}",
            )
            return render(
                request,
                "error.html",
                {"message": "Failed to set public access for the folder."},
            )

    else:
        messages.error(
            request, f"Failed to create folder on Google Drive. Error: {response.text}"
        )
        return render(
            request,
            "error.html",
    ...
    for folder in folders:
        ...
        if response.status_code == 200:
            ...
            if new_folder_name != folder.folder_name:
                ...
                folder.save()
                messages.success(
                    request, f"Folder '{folder.folder_name}' updated successfully."
                )
                sync_success = True
            ...
            folder.delete()
            messages.info(
                request, f"Folder '{folder.folder_name}' was deleted from Google Drive."
            )
            sync_success = True
            ...
            messages.error(
                request,
                f"Failed to sync folder '{folder.folder_name}': {response.status_code}",
            )
